...
class Swimming_Laps_Job(BaseETLJob):
    ...
    def load(self, transformed_data: List[SwimmingLap]) -> Dict[str, Any]:
        ...
        # insert the swimming laps into the database
        # close the database connection
        with self.database_client.get_connection() as conn:
            cursor = conn.cursor()
            cursor.executemany("""
                INSERT OR REPLACE INTO raw_swimming_laps (
                    ...
                    total_strokes, average_strokes, average_swolf, average_stroke_distance, swim_drill, interval_type
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, laps_data)
            conn.commit()
